...
using cv::cuda::GpuMat; // use cuda gpu matrix to accelerate opencv
...
namespace kinectfusion {
    ...
    struct CameraParameters {
        ...
        {
            if (level == 0) return *this; // if level is 0, return the original image -> *this to get itself.

            const float scale_factor = powf(0.5f, static_cast<float>(level)); // 0.5f ^ level -> 2 ^ (-level)
            // return down-sampled camera. + 0.5f is used to do the ceiling function manually.
    ...
    struct GlobalConfiguration {
        ...
        bool use_output_frame = { true }; // some switch to control input frame's origin.
    ...
    namespace internal {
        ...
        struct FrameData {
            ...
            // movement constructor for control movement
            ...
            // movement assignment for control movement
        ...
        struct CloudData {
                    ...
                    host_vertices{}, host_normals{}, host_color{}, point_num{nullptr}, host_point_num{}
            {
                ...
                cudaMalloc(&point_num, sizeof(int)); // hold and control by cuda memory
                cudaMemset(point_num, 0, sizeof(int)); // set it to be 0, point_num on cuda memory
            }
            ...
            void download()
            {
                // down-load the data from cuda to host
                vertices.download(host_vertices);
                normals.download(host_normals);
                color.download(host_color);

                cudaMemcpy(&host_point_num, point_num, sizeof(int), cudaMemcpyDeviceToHost); // refresh host_point_num to synchronize the data.
            }
        };
