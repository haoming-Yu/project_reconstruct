    ...
    namespace internal {
        namespace cuda {
                                            ...
                                            const PtrStepSz<int> number_vertices_table)
            {
                ...
                if (__all_sync(__activemask(), x >= volume_size.x) || __all_sync(__activemask(), y >= volume_size.y))
                    return;
                ...
                for (int z = 0; z < volume_size.z - 1; ++z) {
                    ...
                    const int total = __popc(__ballot_sync(__activemask(), n_vertices > 0));
                    ...
                    const int offset = binaryExclScan(__ballot_sync(__activemask(), n_vertices > 0));
